...
attempt to use diffusers scheduler

- ei oikein toimi vielä
- epäselvää miten tässä hoitaa ohjaus cond_fn:llä


edistystä: key-nimet talletettussa modelissa ei täsmänneet, ylimääräinen denoise_fn, nyt poistettu
mekanismit alkavat jotenkin toimia mutta diffuusio ei edelleenkään mene oikein

nyt toimii, kun laitoin samanlaiset betat kuin omassa versiossa

ohjaukset ei vielä toimi, paitsi img2img skip

nyt toimii CLIP-ohjauskin

'''

from cutouts_safe import cut
...
parser.add_argument('--rounds', type=int, default=1, help='')
...
parser.add_argument('--laplace', nargs=2, default=[0, 0], help='')

opt = parser.parse_args()

opt.laplace_radius = int(opt.laplace[0])
opt.laplace_alpha = float(opt.laplace[1])
...
if 'attn_config' in data:
    opt.attn_config = data['attn_config']

elif 'opt' in data and hasattr(data['opt'], 'attn_config'):
    opt.attn_config = data['opt'].attn_config

elif 'opt' in data and hasattr(data['opt'], 'attn'):
    opt.attn_config = parse_attn_config(data['opt'].attn)

else:
    opt.attn_config = None


if mtype == "tinyunet_conf_attention":
    model = Unet(
        dim=64,
...
def cond_fn(x, t, x_s):
    ...
    #print("x_s", x_s.min(), x_s.max())

    if opt.text != "" and opt.textw > 0:
        #nimg = x_s.clip(-1, 1) + 0.5
        nimg = (x_s.clip(-1, 1) + 1) * 0.5
        #print("nimg1", nimg.min(), nimg.max())    
        nimg = cut(nimg, cutn=opt.cutn, low=opt.low, high=opt.high, norm = cnorm)
        #print("nimg2", nimg.min(), nimg.max())
...
def getx(ifn=None):
    ...
    if ifn != None:   
        ...
        if opt.contrast != 1:
            x = adjust_contrast(x, opt.contrast)
        if opt.gamma != 1:
            x = adjust_gamma(x, opt.gamma)
        #if opt.brightness != 0:
        #    imT = adjust_brightness(imT, opt.brightness)
        if opt.saturation != 1:
            x = adjust_saturation(x, opt.saturation)
        x -= 0.5 #(imT_ * 2) - 1
...
def scale_timesteps(t):
    return t.float() * (opt.steps / opt.trainsteps)   
'''

#bs = 1
ctr = 0
for k in range(0, opt.rounds):    
    for inp in inputlist:
          ...
          if (opt.text!="" and opt.textw > 0):
             with torch.enable_grad():
                 with torch.autocast(device_type='cuda', dtype=torch.float16):
                     x.requires_grad_() 
                     model_out = model(x, my_t).cuda() 
                     
                     alpha_prod_t = scheduler.alphas_cumprod[t]
                     beta_prod_t = 1 - alpha_prod_t

                     if opt.pred == "x0":
                        pred_original_sample = model_out
                        noise = (x - alpha_prod_t.sqrt() * pred_original_sample) / beta_prod_t.sqrt()
                     else:
                         noise = model_out
              ...
              with torch.no_grad():
                  with torch.autocast(device_type='cuda', dtype=torch.float16):
                        ...
                        if opt.pred == "x0":
                            alpha_prod_t = scheduler.alphas_cumprod[t]
                            beta_prod_t = 1 - alpha_prod_t
                            noise = (x - alpha_prod_t.sqrt() * model_out) / beta_prod_t.sqrt()
                        else:
                            noise = model_out
      ...
      #im = (x.clone()+0.5) #.clamp(0,1)
      im = ((x.clone()+1) / 2).clamp(0,1)
     
      if opt.postproc:
          im = pprocess(im.cpu(), opt) #*2 - 1, opt)
          im -= im.min()
          im /= im.max()
          save_image((im), opt.dir+"/"+oname+"-"+str(ctr)+"-finalp.png")    
      
      else:
          save_image(im, opt.dir+"/"+oname+"-"+str(ctr)+"-final.png")
