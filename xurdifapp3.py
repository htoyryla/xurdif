
# urdiffusion gradio app

# this version supports also minidiffusion models (i.e. model types other than unet2 and layers/mults configs)

from diffusers import DDIMScheduler

import torch
from torchvision.utils import save_image
from torchvision import transforms
import torch.nn.functional as F
import torchvision.transforms.functional as TF
from PIL import Image
from tqdm import tqdm
import os
import clip
import argparse
import cv2

from postproc26 import pprocess
from functools import partial
import numpy as np
import random

#from alt_models.Unet2 import Unet 

import types
import gradio as gr
#from cutouts import cut
import math
from cutouts25 import CutoutConfig, GpuCutoutSampler


# set up initial values

opt = types.SimpleNamespace()

opt.load = "md2un2-msgbox-624.pt"
opt.mtype= None

opt.text = ""
opt.image = ""
opt.img_prompt = ""
opt.tgt_image = ""
opt.lr = 10.
opt.ssimw = 0
opt.textw = 0
opt.imgpw = 0
opt.tdecay = 1

opt.mul = 1

opt.trainsteps = 1000
opt.modelSize = 512

opt.skip = 0
opt.ema = True
opt.imageSize = 768
opt.h = 0
opt.w = 0
opt.mults = [1, 1, 2, 2, 4, 4, 8, 8]
opt.spher = False

opt.steps = 50
opt.eta = 0.5

opt.cutn = 32
opt.low = 0.4
opt.high = 0.97

opt.weak = 0

opt.postproc = True
opt.onorm = True
opt.contrast = 0.8
opt.saturation = 1.
opt.gamma = 1.
opt.eqhist = 0.5
opt.unsharp = 1.
opt.c1 = 0.
opt.c2 = 1.
opt.sharpenlast = True
opt.sharpkernel = 3
opt.median = 0
opt.ovl0 = 0
opt.noise = 0.02
opt.bil = 0.
opt.bils1 = 75
opt.bils2 = 75


if opt.h == 0:
    opt.h = opt.imageSize

if opt.w == 0:
    opt.w = opt.imageSize
    
steps = opt.steps

# set up clip model for textual guidance


perceptor, clip_preprocess = clip.load('ViT-B/32', jit=False)
perceptor = perceptor.eval()
cnorm = transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))
  

print("CLIP param dtype:", next(perceptor.parameters()).dtype)


# get a list of available models

dc = os.listdir("./models/")
ml = []
for d in dc:
        if ".pt" in d:
          mn = d.split("/")[-1]
          ml.append(mn)
ml.sort()          
opt.modellist = ml    

#opt.load = ml[0]

if opt.load in opt.modellist:
    fn = "./models/"+opt.load
else:
    opt.load = ml[0]
    fn = "./models/"+opt.load
        
# function to load model weights  
  
def load_model(fn):
  data = torch.load(fn)

  try:
    print("loaded "+fn+", correct mults: "+",".join(str(x) for x in data['mults']))
  except:
    print("loaded "+fn+", no mults stored")

  m = "ema" if opt.ema else "model"
  dd = data[m].copy()
  
  # if using DDIM remove original scheduler steps
  
  if opt.steps < dd['betas'].shape[0]:
    sched_keys = ['betas', 'alphas_cumprod', 'alphas_cumprod_prev', 'sqrt_alphas_cumprod', 'sqrt_one_minus_alphas_cumprod', 'log_one_minus_alphas_cumprod', 'sqrt_recip_alphas_cumprod', 'sqrt_recipm1_alphas_cumprod', 'posterior_variance', 'posterior_log_variance_clipped', 'posterior_mean_coef1', 'posterior_mean_coef2']
    for k in sched_keys:
       del dd[k]

  return dd, data
  
# load default model  
  
dd, data = load_model(fn)
    
dd_ = {}
for k in dd.keys():
    v = dd[k]
    k_ = k.replace("denoise_fn.","")
    dd_[k_] = v  

#print(data.keys())
        
if 'mtype' in data:
    mtype = data['mtype']
elif 'opt' in data: 
    mtype = data['opt'].model
else:
    mtype = "unet2"
    print("model type not given, assuming unet2")    
    
    
if mtype == "unet0":
  from alt_models.Unet0 import Unet
elif mtype == "unet0k5":
  from alt_models.Unet0k5 import Unet
elif mtype == "unet1":
  from alt_models.Unet1 import Unet
elif mtype == "unet2":
  from alt_models.Unet2 import Unet    
elif mtype == "unetcn0":    
  from alt_models.UnetCN0 import Unet
elif mtype == "tinyunet":
  from alt_models.tinyunet import TinyUNet as Unet 
elif mtype == "tinyunet_with_attention":
  from alt_models.tinyunet_with_attn import TinyUNetWithAttn as Unet   
elif mtype == "tinyunet_with_attention3":
  from alt_models.tinyunet_with_attn3 import TinyUNetWithAttn as Unet   
  
# initialize model and diffusion

opt.mults = data['mults']

# --- NEW: detect prediction type (eps / x0) ---
if 'pred' in data:
    opt.pred = data['pred']          # e.g. "eps" or "x0"
elif 'opt' in data and hasattr(data['opt'], 'pred'):
    opt.pred = data['opt'].pred
else:
    opt.pred = "eps"                 # default for older models
    print("pred type not given, assuming 'eps'")

print("Prediction type:", opt.pred)    

model = Unet(
    dim = 64,
    dim_mults = opt.mults # (1, 2, 4, 8)
).cuda()
  
model.load_state_dict(dd_, strict=False)

current_modelname = fn.split(os.sep)[-1]
opt.modelname = current_modelname
opt.modeltype = mtype

bs = 1

def spherical_dist_loss(x, y):
    x = F.normalize(x, dim=-1)
    y = F.normalize(y, dim=-1)
    return (x - y).norm(dim=-1).div(2).arcsin().pow(2).mul(2)     

transform = transforms.Compose([transforms.Resize((opt.h, opt.w)), transforms.ToTensor()]) 

tensor_to_pil = transforms.ToPILImage()

    
# important! will not work with diffusers default betas 

def make_betas(timesteps):
    s = 0.008
    steps = timesteps + 1
    x = torch.linspace(0, timesteps, steps)
    alphas_cumprod = torch.cos(((x / steps) + s) / (1 + s) * math.pi * 0.5) ** 2
    alphas_cumprod = alphas_cumprod / alphas_cumprod[0]
    betas = 1 - (alphas_cumprod[1:] / alphas_cumprod[:-1])
    betas = torch.clip(betas, 0, 0.999)
    
    return betas.numpy()

# create scheduler    

scheduler = DDIMScheduler(num_train_timesteps=opt.trainsteps, prediction_type = "epsilon", trained_betas = make_betas(opt.trainsteps), clip_sample=False)
scheduler.set_timesteps(opt.steps, device="cuda")

scheduler.alphas_cumprod = scheduler.alphas_cumprod.to("cuda")

timesteps = None

# function to set timesteps
       
def get_timesteps(skip):
    skip = min(skip, opt.steps)
    timesteps = scheduler.timesteps[skip:]
    t_start = max(opt.steps - skip, 0)
    return {'timesteps': timesteps, 't_start': t_start}

# diffusion run to be run by each user instance

def diffusion_run(modelsel, im, o,  prompt_img = None):

    # only global objects are the currently active model and its name    
    
    global current_modelname, model 

    o.h = o.imageSize
    o.w = o.imageSize

    #cutoutSlider = o.cuts

    run_timesteps = get_timesteps(o.skip)['timesteps']
    print(o.skip, run_timesteps[0], run_timesteps[-1])
    
    # Setup once:
    cfg = CutoutConfig(
        base_low=64,
        base_high=o.imageSize,          # or current image shorter side
        num_min=4,
        num_max=64,
        strategy="mixture",     # try "loguniform" too
        collapse_range_at_right=True,
        margin_frac=0.02,
        out_size=224,
        clip_normalize=True,
    )

    cutoutSampler = GpuCutoutSampler(cfg)
    

    # load model weights unless same model is already loaded
    
    o.modelname = modelsel
    
    #print(current_modelname, o.modelname)        
    
    if o.modelname != current_modelname:
        fn = "models"+os.sep + o.modelname
        print("loading "+fn)
        dd, data = load_model(fn)
    
        dd_ = {}
        for k in dd.keys():
            v = dd[k]
            k_ = k.replace("denoise_fn.","")
            dd_[k_] = v  

        #print(data.keys())
        if 'mtype' in data:
            mtype = data['mtype']
        elif 'opt' in data: 
            mtype = data['opt'].model
        else:
            mtype = "unet2"
            print("model type not given, assuming unet2")     
            
        if mtype != opt.mtype: 
            if mtype == "unet0":
              from alt_models.Unet0 import Unet
            elif mtype == "unet0k5":
              from alt_models.Unet0k5 import Unet
            elif mtype == "unet1":
              from alt_models.Unet1 import Unet
            elif mtype == "unet2":
              from alt_models.Unet2 import Unet    
            elif mtype == "unetcn0":    
              from alt_models.UnetCN0 import Unet
            elif mtype == "tinyunet":
              from alt_models.tinyunet import TinyUNet as Unet
            elif mtype == "tinyunet_with_attention":
              from alt_models.tinyunet_with_attn import TinyUNetWithAttn as Unet 
            elif mtype == "tinyunet_with_attention3":
              from alt_models.tinyunet_with_attn3 import TinyUNetWithAttn as Unet   
       
            
            opt.modeltype = mtype            
        
         # --- NEW: detect prediction type (eps / x0) ---
        if 'pred' in data:
            opt.pred = data['pred']          # e.g. "eps" or "x0"
        elif 'opt' in data and hasattr(data['opt'], 'pred'):
            opt.pred = data['opt'].pred
        else:
            opt.pred = "eps"                 # default for older models
            print("pred type not given, assuming 'eps'")

        print("Prediction type:", opt.pred)    

        model = Unet(
                dim = 64,
                dim_mults = data['mults'] # (1, 2, 4, 8)
            ).cuda()
              
        print("Model type: "+mtype)
        model.load_state_dict(dd_, strict=False)
        current_modelname = fn.split(os.sep)[-1]
         
    # cond function for guidance by text prompt and images

    @torch.enable_grad()
    def cond_fn(x, t, x_s):
        nonlocal prompt_img #, cutoutSampler

        x_is_NaN = False
        #x.grad = None
        #x.requires_grad_()
        n = x.shape[0]         
    
        x_s.requires_grad_()
        x_grad = torch.zeros_like(x_s)
                    
        loss = 0
        losses = []

        nimg = None

        if o.text != "" and o.textw > 0:
            nimg = (x_s.clip(-1, 1) + 1) * 0.5
            nimg = nimg.float()
            #nimg = cut(nimg, cutn=o.cutn, low=opt.low, high=o.high, norm = cnorm)
            nimg = cutoutSampler.sample(nimg, slider = o.cuts)
            nimg = nimg.float()
            #print("nimg", nimg.shape, nimg.min(), nimg.max())   
            
            # get image encoding from CLIP
            with torch.autocast(device_type='cuda', enabled=False):
                img_enc = perceptor.encode_image(nimg).float() 

                txt_enc_f = txt_enc.detach().float()

                if o.spher:
                    loss = o.textw * spherical_dist_loss(txt_enc_f, img_enc).mean()
                else:
                    loss = o.textw * (1 - torch.cosine_similarity(txt_enc_f, img_enc)).mean()
  
                losses.append(("Text loss", float(loss.detach().cpu()))) 
            
            #if o.tdecay < 1.:
            #    o.textw = o.tdecay * o.textw
            x_grad += torch.autograd.grad(loss.sum(), x_s, retain_graph = True)[0]


        if prompt_img is not None:
            if nimg == None:
                nimg = (x_s.clip(-1, 1) + 1) * 0.5     
                nimg = cutoutSampler.sample(nimg, slider = o.cuts)
            
            with torch.autocast(device_type='cuda', enabled=False):
                img_enc = perceptor.encode_image(nimg)
                del nimg    
                if o.spher:
                    loss1 = o.imgw * spherical_dist_loss(imgp_enc, img_enc).mean()
                else:    
                    loss1 = o.imgw * (1-torch.cosine_similarity(imgp_enc, img_enc)).view(-1, bs).T.mean(1)
                losses.append(("Img prompt loss",loss1.item())) 
                loss = loss + loss1     
        
            x_grad += torch.autograd.grad(loss1.sum(), x_s, retain_graph = True)[0]
           
        
        if torch.isnan(x_grad).any():
             return torch.zeros_like(x)             
        
        #print("grad norm loss", x_grad.norm().item(), losses) 

        del x, x_s, loss
        
        #with torch.no_grad():
        #    print("losses:", losses)
        #    print("x_grad rms:", x_grad.flatten(1).pow(2).mean(1).sqrt())
        #    print("x_grad max:", x_grad.abs().flatten(1).max(1).values)
          
        return x_grad.detach()
        
        
    # get text encoding for the prompt    

    if o.text != "" and o.textw > 0:
        tx = clip.tokenize(o.text)                        # convert text to a list of tokens 
        txt_enc = perceptor.encode_text(tx.cuda()).detach()   # get sentence embedding for the tokens
        del tx
    else:
        txt_enc = None    
    
    # function to initialize diffusion with noise & init image (when applicable)
        
    def getx(im=None):
        init_noise = torch.zeros(bs,3,o.h,o.w).normal_(0,1).cuda()
        
        # if init image, mix it with noise
        
        if im is not None:   
            tf = transforms.Compose([transforms.Resize((o.h, o.w)), transforms.ToTensor()]) 
            im = Image.fromarray(im)
            x = tf(im).cuda().unsqueeze(0)
            x = (x * 2) - 1
            print("getx", x.min(), x.max())

            print("getx w ", o.weak, run_timesteps[0], len(run_timesteps))
            #x = o.mul*scheduler.add_noise(x, init_noise, timesteps[o.weak])
            x = o.mul*scheduler.add_noise(x, init_noise, run_timesteps[0])
        
        # otherwise pure noise adjusted to proper level
        
        else:
            x = o.mul*init_noise * scheduler.init_noise_sigma

        return x   

    # main diffusion loop starts here
    
    #timesteps = get_timesteps(o)['timesteps'] 

    #print(len(timesteps), "timesteps")

    #progress(0)

    ctr = 0
    
    # get initial noise (mixed with init image if present)
    
    x = getx(im).cuda()     

    print(x.std())
    
    if prompt_img is not None:   
        #imP = transform(Image.open(opt.img_prompt).convert('RGB')).float().cuda().unsqueeze(0)
        im = Image.fromarray(prompt_img)
        im = transform(im).cuda().unsqueeze(0)
        print("init im before clip(0,1)", im.min(), im.max() )
        nimg = im.clip(0,1)
        #nimg = cut(nimg, cutn=o.cutn, low=o.low, high=o.high, norm = cnorm)
        nimg = cutoutSampler.sample(nimg, slider = o.cuts)
        #print(nimg.shape)
        imgp_enc = perceptor.encode_image(nimg.detach()).detach() 
    
    # iterate through timesteps
    
    #for i in tqdm(timesteps):
    for i in run_timesteps:
        
        with torch.no_grad():
            t = torch.tensor([i] * bs, device='cuda').detach()
            
            if (o.text != "" and o.textw > 0) or (prompt_img is not None and o.imgw > 0):
                with torch.enable_grad():
                    x.requires_grad_()
                    t = t.to(device=x.device)

                    # -----------------------------
                    # 1) UNet forward in fp16
                    # -----------------------------
                    with torch.autocast(device_type='cuda', dtype=torch.float16):
                        pred = model(x, t)   # keep raw output; don't .to(x.dtype) yet

                    # -----------------------------
                    # 2) Guidance math in fp32
                    # -----------------------------
                    with torch.autocast(device_type='cuda', enabled=False):
                        x_f    = x.float()
                        pred_f = pred.float()

                        # NOTE: keep these in fp32
                        alpha_prod_t = scheduler.alphas_cumprod[t.cpu()].to(device=x.device, dtype=torch.float32)
                        beta_prod_t  = 1.0 - alpha_prod_t

                        alpha = alpha_prod_t.view(-1, 1, 1, 1)
                        beta  = beta_prod_t.view(-1, 1, 1, 1).clamp(min=1e-8)

                        # Handle ε vs x0 (all fp32)
                        if opt.pred == "eps":
                            eps_f = pred_f
                            x0_f  = (x_f - beta.sqrt() * eps_f) / alpha.sqrt()
                        else:  # opt.pred == "x0"
                            x0_f  = pred_f
                            eps_f = (x_f - alpha.sqrt() * x0_f) / beta.sqrt()

                        # CLIP guidance gradient dL/dx0 (keep fp32!)
                        grad_x0 = cond_fn(x_f, t, x0_f)  # cond_fn returns detached tensor already
                        grad_x0 = torch.nan_to_num(grad_x0, nan=0.0, posinf=0.0, neginf=0.0)

                        # RMS normalization (usually better for CLIP than full L2 over all pixels)
                        g_rms = grad_x0.flatten(1).pow(2).mean(dim=1).sqrt().view(-1, 1, 1, 1) + 1e-8
                        grad_x0 = grad_x0 / g_rms

                        # beta is in [~1 .. 0] across the process (depending on schedule)
                        # make a [0..1] "signal strength" proxy
                        signal = alpha.sqrt()          # ~0 early, ~1 late
                        # or: signal = (alpha / (alpha + beta)).sqrt()

                        # choose an exponent to control how late it kicks in
                        p = getattr(o, "gpower", 2.0)  # 1..4 is typical

                        gs_eff = o.gs * (signal ** p)

                        # grad_x0 is dL/dx0 (same for both model types)
                        # take a step in x0 space
                        x0_f = x0_f - gs_eff * grad_x0   # minus = gradient descent on loss

                        # recompute eps from modified x0 (this is the key compatibility step)
                        eps_f = (x_f - alpha.sqrt() * x0_f) / beta.sqrt()

                        # cast back
                        eps = eps_f.to(x.dtype)

                    # -----------------------------
                    # 4) DDIM step (unchanged)
                    # -----------------------------
                    s = scheduler.step(eps, t, x, eta=opt.eta)
                    x = s['prev_sample'].cuda().detach()
                    x_s = s['pred_original_sample'].detach()

                    #del sample, grad_x0, alpha_prod_t, beta_prod_t
            else:              
                with torch.no_grad():
                    with torch.autocast(device_type='cuda', dtype=torch.float16): 
                       x.requires_grad_() 
                       t = t.to(device=x.device)

                       # raw model output
                       pred = model(x, t).to(x.dtype)
                       
                       alpha_prod_t = scheduler.alphas_cumprod[t.cpu()].to(device=x.device, dtype=x.dtype)
                       beta_prod_t = 1 - alpha_prod_t

                       # Handle ε vs x0
                       if opt.pred == "eps":
                            eps = pred
                            # convert to x0 for convenience (guidance uses x0-space)
                            pred_original_sample = (x - beta_prod_t.sqrt() * eps) / alpha_prod_t.sqrt()

                       else:  # opt.pred == "x0"
                            x0_pred = pred
                            pred_original_sample = x0_pred
                            # convert x0 → ε for scheduler
                            eps = (x - alpha_prod_t.sqrt() * x0_pred) / beta_prod_t.sqrt()
                                    
                       s = scheduler.step(eps, t, x, eta=opt.eta) #
                       x = s['prev_sample'].cuda().detach() 
                       x_s = s['pred_original_sample'].detach() 
                       del alpha_prod_t, beta_prod_t
            #del noise
            
            # adjust output into raw (unprocessed) image and a post processed one

            #xs = x_s.detach()
            #print("x_s preclamp  min max mean std", xs.min().item(), xs.max().item(), xs.mean().item(), xs.std().item())

            #xs_c = xs.clamp(-1, 1)
            #print("x_s clamped   min max mean std", xs_c.min().item(), xs_c.max().item(), xs_c.mean().item(), xs_c.std().item())
            
            im = (x_s.clone().clamp(-1,1) + 1) * 0.5
  
            im = torch.nan_to_num(im, nan=0.0, posinf=1.0, neginf=0.0)

            #print("im min max mean std", im.min().item(), im.max().item(), im.mean().item(), im.std().item())

            #if o.onorm:
            #    im -= im.min()
            #    im /= im.max()
            #else:    
            #  im = im.clamp(0,1)
             
            # robust-ish contrast based on mean/std
            m = im.mean()
            s = im.std().clamp(min=1e-6)
            target_std = 0.18  # in [0,1] space, tune
            im = (im - m) * (target_std / s) + 0.5
            print("im min max mean std", im.min().item(), im.max().item(), im.mean().item(), im.std().item())
            im = im.clamp(0,1) 

            imout_raw = tensor_to_pil(im[0].clone())  
           
            im = pprocess(im*2 - 1, o)
            im -= im.min()
            im /= im.max()
            
            imout = tensor_to_pil(im[0])
            
            ctr += 1
            
            #progress(float(i) / (o.steps - o.skip))
        
            yield imout, imout_raw, str(ctr)+"/"+str(o.steps - o.skip)
            
# gradio UI           
      
with gr.Blocks() as demo:
    
    imo = gr.State(Image.new('RGB', (1, 1)))
    opt_ = gr.State(opt)
    image_internal = gr.State(Image.new('RGB', (1, 1)))
    #opt_ = opt
    opt_.modelname = current_modelname
    
    with gr.Row():
        with gr.Column(min_width = 80):
            html = '<div style="font-size: 36px; color:#666666;">#xurdif</div><br>'+  \
            '<div style="font-size: 14px; color:#666666;">by @htoyryla 2025</div>'
            logo = gr.HTML(html)
            text_input = gr.Textbox(label="Prompt")
            textw = gr.Slider(minimum=0., maximum=100., value=10., label="Text weight")
            guidance = gr.Slider(minimum=-0., maximum=0.3, value=0.02, label="Guidance step")
            gpower = gr.Slider(minimum=-0., maximum=4., value=1., label="Guidance power")
            spher = gr.Checkbox(label="Spher")
        with gr.Column():
            init_image = gr.Image(label="Init")
            skip = gr.Slider(minimum=0, maximum=opt.steps, value=0, step=1, label="Skip")
            mul = gr.Slider(minimum=0., maximum=1., value=1., label="Noise level")
            weak = gr.Slider(minimum=0., maximum=1., value=0., label="Simplify")
        with gr.Column():    
            prompt_img = gr.Image(label="Image prompt")
            imgw = gr.Slider(minimum=0., maximum=300., value=10., label="Img prompt weight")            
        with gr.Column():
            modelsel = gr.Dropdown(choices = opt.modellist, value=opt.load, label="Select model") 
            imageSize = gr.Dropdown(choices = [512, 768, 1024, 1280], value=opt.imageSize, label="Image size", interactive=True)       
            cuts = gr.Slider(minimum=0., maximum=1., value=0.5, step=0.05, label="Detail - Structure")            
            text_button = gr.Button("Go")
            process_status = gr.Textbox(label="Generation status")        
    with gr.Row():    
        image_output_raw = gr.Image(label="generated")
        image_output = gr.Image(label="postprocessed")
    
    with gr.Row():    
        with gr.Column():
            contrast = gr.Slider(minimum=0.5, maximum=2., value=1., label="Contrast")
            gamma = gr.Slider(minimum=0.5, maximum=2., value=1., label="Gamma")
            saturation = gr.Slider(minimum=0., maximum=2., value=1., label="Saturation")
        with gr.Column():
            eqhist = gr.Slider(minimum=0., maximum=4., value=0.5, label="Eq hist")
            unsharp = gr.Slider(minimum=0., maximum=4., value=1., label="Sharpen")
            noise = gr.Slider(minimum=0., maximum=0.1, value=0.02, label="Noise")
        with gr.Column():
            bil = gr.Slider(minimum=0., maximum=100, value=0, step=1, label="Bilateral blur")
            bils1 = gr.Slider(minimum=3, maximum=177, value=75, step=2, label="Sigma space")
            bils2 = gr.Slider(minimum=3, maximum=177, value=75, step=2, label="Sigma color")
        with gr.Column():
            proc_button = gr.Button("Change")
            post_process_status = gr.Textbox(label="Postprocess status")
    
    # function to apply changed post processing settings to current image
   
    def pproc(o, imo, contrast, gamma, saturation, eqhist, unsharp, noise, bil, bils1, bils2):
        
        o.contrast = float(contrast)
        o.gamma = float(gamma)
        o.saturation = float(saturation)
        o.eqhist = float(eqhist)
        o.unsharp = float(unsharp)
        o.noise = float(noise)
        
        if bil > 0:
            bil = int(bil*2) + 1
        o.bil = int(bil)
        o.bils1 = int(bils1)    
        o.bils2 = int(bils2)    
        
        post_process_status.value = "Postprocessing..."
        print(imo)
        imT = TF.to_tensor(imo).unsqueeze(0)*2 - 1
        imT = pprocess(imT, o)
        imT = imT - imT.min()
        imT = imT / imT.max()
        imout = TF.to_pil_image(imT[0])
        post_process_status.value = "Done"
        
        return imout, o, "Done"
        
    # function to run diffusion based on current settings    
    
    def run(o, imo, ms, t, s, m, im_in, skip, weak, prompt_img, imgw, imageSize, cuts, spher, guidance, gpower):
        
        o.modelname = modelsel
        o.textw = float(s)
        o.mul = float(m)
        o.text = t
        o.skip = int(skip)
        o.weak = int(weak*(o.steps - o.skip - 1))
        o.imgw = float(imgw)
        o.imageSize = int(imageSize)
        o.cuts = float(cuts)
        o.spher = spher
        o.gs = guidance
        o.gpower = gpower
        
        process_status.value = "Processing..."
        for im, imr, status in diffusion_run(ms, im_in, o, prompt_img):
            #print(i)
            imo.value = imr
            process_status.value = status
            # Yield immediately for Gradio to update UI (if needed)
            yield im, imr, imr, imo, o, status

        return im, imr, imr, imo, o, status    
            
    # buttons to trigger diffusion and post processing        
             
    text_button.click(fn=run,  concurrency_limit=1, inputs=[opt_, imo, modelsel, text_input, textw, mul, init_image, skip, weak, prompt_img, imgw, imageSize, cuts, spher, guidance, gpower], outputs=[image_output, image_output_raw, image_internal, imo, opt_, process_status])
    
    proc_button.click(concurrency_limit=1, show_progress=False, fn=pproc, inputs=[opt_, image_internal, contrast, gamma, saturation, eqhist, unsharp, noise, bil, bils1, bils2], outputs=[image_output, opt_, post_process_status])


#demo.queue(concurrency_count=1)
demo.launch(server_name = "0.0.0.0", server_port=9777) #, auth=("hannu", "fc49wz"))
